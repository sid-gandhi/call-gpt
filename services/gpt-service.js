...
const { getVectorStoreSearchResults } = require("../lib/vector-store");
const { getPineconeClient } = require("../lib/pinecone-client");
...
class GptService extends EventEmitter {
  constructor() {
    ...
    (this.userContext = [
      {
        role: "system",
        content: `You are an Bill Ackman like investment analyst. You have a confident personality. 
      ...
      },
      {
        role: "assistant",
        content:
          "Hello! I understand you're looking advice on investing in the stock market. How can I help you today?",
      },
    ]),
  ...
  async completion(text, interactionCount, role = "user", name = "user") {
    ...
    // RAG to fetch relevant context from the vector store
    const client = await getPineconeClient();
    const { context, sources, sourceAndContext } =
      await getVectorStoreSearchResults(client, text, "nvidia.txt");

    this.userContext.push({ role: "system", content: context });
